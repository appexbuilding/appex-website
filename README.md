# hugofy-uikit-boilerplate

## This boilerplate is copyrighted and protected. Built by Sam Brunno, Canberra.

### If you are wanting to use this codebase, you have no permission to re-use unless you get granted rights and permission by Sam.
...
We use the [html-loader](https://github.com/webpack-contrib/html-loader) to export HTML as a string and minify the output. This allows you to import your `src/index.html` within your `src/index.ts`. We can simply minify the HTML with a loader option `minimize: true`, so this is why we leave it in the `webpack.common.js` instead of moving it to `webpack.prod.js`.
...
plugins: [
  new HtmlWebpackPlugin({
    title: 'appex-webpack-boilerplate',
    filename: 'index.html',
...
Majority of the component SCSS comes from [UIkit](https://getuikit.com/). While the folder structure and SCSS approach is based on [ITCSS](https://www.xfive.co/blog/itcss-scalable-maintainable-css-architecture/). This combination allows us to quickly skin pre-built components and build new components in a easy, consistent and scalable way.

When modifying base UIKit component, remember to use the approach outlined in the [documentation](https://getuikit.com/docs/sass). Start by first modifying the variables that you have access to through UIkit (you may need to go digging in your `node_modules`). Then move onto more specific SCSS by using their mixin hook system. If you require a highly custom component based off a base version, create a file under the `components` folder with the new variant and create your styling there instead of in the mixin hook. All UIKit related styling should be done in the `theme` folder, which each component having its own folder with a `variables` and `mixin` scss file. Remember to import these into the `main.scss` file. Keeping these import separate also allows you to only include the component overrides that you really need.

#### Improvements that are needed

- ITCSS was added after the fact, so there might be some inconsistencies to the pattern, feel free to refine the setup and open a PR to solve this
- We are also including the whole of UIkit into the project. Ideally we'd want a similar setup to the theme overrides where we can add/remove components as needed.
- All the module scss is currently in the `site/layout/partials/modules` folder. It would be good to move them to the main scss file and also allowing a developer to add/remove modules as needed.
...
This plugin is intended to provide an offline experience for __webpack__ projects. It uses __ServiceWorker__, and __AppCache__ as a fallback under the hood. We simply include this plugin in our `webpack.prod.js`, and the accompanying runtime in our client script (src/index.ts), and our project will become offline ready by caching all (or some) of the webpack output assets.
